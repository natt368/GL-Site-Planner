...
export const CableEstimatorView: React.FC<CableEstimatorViewProps> = ({
  ...
  const isHopper = !!activeBin?.isHopper;
  const C = isHopper ? parseFloat(activeBin?.hopperConeHeight || '8') || 8 : 0;
  ...
  const cp = C * pixelsPerFoot; // Hopper cone height in pixels
  const gcp = GC * pixelsPerFoot; // 3ft ground clearance in pixels
  const coney = gy - gcp; // Bottom of hopper cone (3ft above ground gy)
  const sgy = coney - cp; // Sidewall bottom Y (top of hopper cone)
  const wl = cx - wp / 2;
  const wr = cx + wp / 2;
  const ey = sgy - hp;
  const py = sgy - tp;
  ...
  const getSnappedCoords = (rawX: number, rawY: number) => {
    ...
    if (snapToFeatures && activeBin) {
      ...
      // - Hopper Cone Bottom (3ft above ground)
      else if (isHopper && Math.abs(y - coney) < snapThreshold) {
        y = coney;
        featureSnapped = true;
      }
          ...
          >
            <g id="viewport-group" transform={`translate(${view.x}, ${view.y}) scale(${view.scale})`}>
              ...
              <g id="dim-layer">
                ...
                {/* Hopper Cone Height Dimension line */}
                {isHopper && (
                  <>
