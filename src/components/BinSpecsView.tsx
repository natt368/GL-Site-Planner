...
export const BinSpecsView: React.FC<BinSpecsViewProps> = ({
  ...
  // Fetch dynamic bin specs from server API if Excel file is updated
  useEffect(() => {
    fetch('/api/bin-specs')
      .then((res) => {
  ...
  return (
    <div id="view-bin-specs" className="flex-1 flex flex-col h-full w-full overflow-hidden bg-black text-neutral-100 select-none">
      {/* Top Header Bar */}
      <header className="bg-neutral-950 border-b border-neutral-900 px-6 py-4 flex flex-wrap items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          {excelSaveStatus && (
            <span className="text-xs font-mono font-bold text-amber-400 bg-amber-500/10 border border-amber-500/30 px-3 py-1 rounded-full animate-pulse flex items-center gap-1.5">
              <Save size={13} />
              {excelSaveStatus}
            </span>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-neutral-950 p-6 space-y-6">
        {/* Search Engine Control Panel */}
        <div className="glass-panel p-5 rounded-2xl border border-neutral-800 bg-neutral-900/90 shadow-xl space-y-4">
          {/* Primary Search Bar */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative flex-1 flex items-center">
              <Search size={20} className="absolute left-4 text-amber-400 pointer-events-none" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search model number or brand e.g. 'Brock 3310', 'Westeel 1505', 'Meridian 1615'..."
                className="w-full pl-12 pr-20 py-3.5 bg-black/80 border-2 border-neutral-800 rounded-xl text-sm font-bold text-white placeholder-neutral-500 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-all"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 text-neutral-400 hover:text-white text-xs font-bold px-2.5 py-1 bg-neutral-800 hover:bg-neutral-700 rounded cursor-pointer transition-all"
                >
                  Clear
                </button>
              )}
            ...
            <button
              ...
              className="px-4 py-3.5 bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-md shrink-0"
            >
              <Plus size={16} />
              Add Custom Spec
            </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-neutral-800">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-xs font-bold text-neutral-200 outline-none focus:border-amber-500"
              >
                {MANUFACTURERS.map((m) => (
            ...
            <div className="flex items-end justify-between px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg">
              <span className="text-[10px] text-neutral-400 font-bold uppercase">Library Matches:</span>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden">
          ...
          <div className="lg:col-span-5 flex flex-col space-y-3 overflow-y-auto pr-1 custom-scrollbar">
            {filteredSpecs.length === 0 ? (
              <div className="p-8 text-center bg-neutral-900/50 border border-neutral-800 rounded-2xl space-y-3">
                <Info size={32} className="mx-auto text-neutral-500" />
                ...
                <button
                  ...
                  className="px-4 py-2 bg-neutral-800 hover:bg-neutral-700 text-xs font-bold rounded-lg text-amber-400"
                >
              ...
              filteredSpecs.map((model) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="px-2 py-0.5 bg-neutral-800 text-amber-400 border border-neutral-700 rounded text-[9px] font-black uppercase tracking-wider">
                            {model.manufacturer}
                          </span>
                          <span className="px-2 py-0.5 bg-neutral-800 text-neutral-300 rounded text-[9px] font-bold uppercase">
                            {model.category}
                          </span>
                        </div>
                        <h3 className="text-sm font-black text-white">{model.fullName}</h3>
                      </div>
                      {isSelected && (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-black/50 p-2.5 rounded-lg text-center text-xs font-mono">
                      <div>
                        <span className="block text-[8px] text-neutral-500 uppercase font-sans">Diameter</span>
                        <span className="font-extrabold text-neutral-200">{model.diameterFt}&apos;</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-neutral-500 uppercase font-sans">Height</span>
                        <span className="font-extrabold text-neutral-200">{model.eaveHeightFt}&apos;</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-neutral-500 uppercase font-sans">Peak Ht</span>
                        <span className="font-extrabold text-neutral-200">{model.totalHeightFt}&apos;</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-neutral-500 uppercase font-sans">Capacity</span>
          ...
          <div className="lg:col-span-7 bg-neutral-900 border border-neutral-800 rounded-2xl p-6 flex flex-col overflow-y-auto custom-scrollbar space-y-6">
            {/* Header Info */}
            <div className="border-b border-neutral-800 pb-4 space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-1 bg-amber-500 text-black font-extrabold text-xs rounded-md uppercase tracking-wider">
                    {selectedSpecModel.manufacturer}
                  </span>
                  <span className="px-2.5 py-1 bg-neutral-800 text-neutral-300 font-bold text-xs rounded-md uppercase">
                    {selectedSpecModel.category}
                  </span>
                  {selectedSpecModel.stiffened && (
                    <span className="px-2 py-1 bg-neutral-800 text-amber-300 border border-amber-500/30 text-[10px] font-bold rounded-md">
                      Stiffened Sidewalls
                    </span>
                  )}
                </div>
                <span className="text-xs font-mono text-neutral-500">Model #: {selectedSpecModel.modelNumber}</span>
              </div>
              <h2 className="text-xl font-extrabold text-white">{selectedSpecModel.fullName}</h2>
              {selectedSpecModel.notes && (
                <p className="text-xs text-neutral-400 leading-relaxed">{selectedSpecModel.notes}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-neutral-950 p-3.5 rounded-xl border border-neutral-800">
                <span className="block text-[9px] text-neutral-500 font-black uppercase">Bin Diameter</span>
                <span className="text-xl font-extrabold font-mono text-white">{selectedSpecModel.diameterFt}&apos;</span>
              </div>
              <div className="bg-neutral-950 p-3.5 rounded-xl border border-neutral-800">
                <span className="block text-[9px] text-neutral-500 font-black uppercase">Eave / Peak Ht</span>
                <span className="text-xl font-extrabold font-mono text-white">
                  {selectedSpecModel.eaveHeightFt}&apos; / {selectedSpecModel.totalHeightFt}&apos;
                </span>
              </div>
              <div className="bg-neutral-950 p-3.5 rounded-xl border border-neutral-800">
                <span className="block text-[9px] text-neutral-500 font-black uppercase">Max Capacity</span>
              ...
              <div className="bg-neutral-950 p-3.5 rounded-xl border border-neutral-800">
                <span className="block text-[9px] text-neutral-500 font-black uppercase">Rings Count</span>
                <span className="text-xl font-extrabold font-mono text-white">
            ...
            <div className="bg-neutral-950 p-5 rounded-xl border border-neutral-800 space-y-4">
              <h4 className="text-xs font-black uppercase text-amber-400 tracking-wider">Full Technical Specifications</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                {(() => {
                  ...
                  return (
                    <div className="flex justify-between border-b border-neutral-800 pb-2 col-span-1 sm:col-span-2">
                      <span className="text-neutral-400 font-bold">Recommended Cables (Dashboard Spec):</span>
                      <span className="text-amber-400 font-mono font-bold">{cableText}</span>
                    </div>
                  );
                })()}
                {selectedSpecModel.isHopper && (
                  <div className="flex justify-between border-b border-neutral-800 pb-2">
                    <span className="text-neutral-400 font-bold">Hopper Cone Ht &amp; Angle:</span>
                ...
                {selectedSpecModel.doorType && (
                  <div className="flex justify-between border-b border-neutral-800 pb-2 col-span-1 sm:col-span-2">
                    <span className="text-neutral-400 font-bold">Door Assembly Type:</span>
                    <span className="text-white font-bold">{selectedSpecModel.doorType}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-neutral-950 p-5 rounded-xl border border-neutral-800 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-neutral-500 font-mono">Persists per bin spec model</span>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="e.g. 38"
                      className="w-full bg-neutral-900 border border-neutral-800 rounded-lg px-3 py-2 pr-8 text-sm text-white font-mono font-bold focus:border-amber-500 outline-none"
                    />
                    <span className="absolute right-3 top-2.5 text-xs text-neutral-500 font-mono font-bold">FT</span>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-neutral-400 uppercase mb-1">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 px-4 bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-lg"
              >
                <Zap size={16} />
              ...
              <button
                ...
                className="flex-1 py-3 px-4 bg-neutral-800 hover:bg-neutral-700 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer border border-neutral-700"
              >
                <Plus size={16} />
      ...
      {showCustomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 max-w-lg w-full space-y-4 shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveCustomModel} className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Manufacturer</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white font-bold outline-none focus:border-amber-500"
                    required
                  />
                </div>
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Model Number</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Diameter (ft)</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Rings</label>
                  <input
                    type="number"
                    value={customSpec.rings}
                    onChange={(e) => setCustomSpec({ ...customSpec, rings: Number(e.target.value) })}
                    className="w-full px-2 py-2 bg-neutral-950 border border-neutral-800 rounded-lg text-white font-bold outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Eave Ht (ft)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-neutral-400 font-bold uppercase mb-1">Capacity (BU)</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-neutral-400 font-bold uppercase mb-1">Notes / Description</label>
                <textarea
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-neutral-800">
                <button
                  type="button"
                  onClick={() => setShowCustomModal(false)}
                  className="px-4 py-2 bg-neutral-800 text-neutral-300 font-bold rounded-lg hover:bg-neutral-700"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-amber-500 text-black font-extrabold rounded-lg hover:bg-amber-400"
                >
