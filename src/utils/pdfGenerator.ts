...
export function drawBinSidePlan(
  ...
  const C = isHopper ? parseFloat(bin.hopperConeHeight || '8') || 8 : 0;
  const GC = isHopper ? 3 : 0; // 3ft ground clearance under hopper cone

  const pixelsPerFoot = 400 / Math.max(H + C + GC, D);
  const cx = 300;
  const gy = 520;
  ...
  const cp = C * pixelsPerFoot;
  const gcp = GC * pixelsPerFoot;
  const coney = gy - gcp;
  const sgy = coney - cp;
  const wl = cx - wp / 2;
  const wr = cx + wp / 2;
  ...
  if (isHopper) {
    // 1. Leg pads & support legs extending from ground gy up to sidewall bottom sgy
    doc.setDrawColor(theme === 'dark' ? 100 : 150);
    doc.setLineWidth(1.5);
    doc.line(mapX(wl), mapY(sgy), mapX(wl), mapY(gy));
    doc.line(mapX(wr), mapY(sgy), mapX(wr), mapY(gy));

    // 2. Core Hopper Bin Shell Outline (cone bottom at coney, 3ft above ground gy)
    doc.setDrawColor(217, 119, 6); // amber-600
    doc.setLineWidth(1.8);
    doc.line(mapX(wl), mapY(sgy), mapX(wl), mapY(ey));
    ...
    doc.line(mapX(wr), mapY(ey), mapX(wr), mapY(sgy));
    doc.line(mapX(wr), mapY(sgy), mapX(cx + cw / 2), mapY(coney));
    doc.line(mapX(cx + cw / 2), mapY(coney), mapX(cx - cw / 2), mapY(coney));
    doc.line(mapX(cx - cw / 2), mapY(coney), mapX(wl), mapY(sgy));

    // Spout flange at coney
    doc.rect(mapX(cx - cw / 2 - 2), mapY(coney), (cw + 4) * scaleX, 6 * scaleY, 'S');
  } else {
    // 1. Foundation Block
...
export async function generateUnifiedPDF(
  ...
  try {
    ...
    // Project Notes Section on Cover Page
    if (project.notes && project.notes.trim()) {
      const dirTableFinalY = (doc as any).lastAutoTable ? (doc as any).lastAutoTable.finalY : 430;
      const notesStartY = Math.max(320, dirTableFinalY + 15);

      if (notesStartY < ph - 50) {
        doc.setFontSize(8);
        doc.setTextColor(110, 110, 110);
        doc.setFont('Helvetica', 'bold');
        doc.text('PROJECT NOTES & INSTRUCTIONS', 50, notesStartY);
        doc.line(50, notesStartY + 5, 460, notesStartY + 5);

        doc.setFontSize(8);
        doc.setFont('Helvetica', 'normal');
        doc.setTextColor(50, 50, 50);

        const splitNotes = doc.splitTextToSize(project.notes.trim(), 390);
        const textHeight = Math.max(30, splitNotes.length * 11 + 10);
        const availableHeight = ph - notesStartY - 30;
        const boxHeight = Math.min(textHeight, availableHeight);

        doc.setFillColor(248, 248, 250);
        doc.rect(50, notesStartY + 12, 410, boxHeight, 'F');
        doc.setDrawColor(225, 225, 228);
        doc.rect(50, notesStartY + 12, 410, boxHeight, 'S');

        doc.text(splitNotes, 60, notesStartY + 26);
      }
    }
